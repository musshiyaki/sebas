...
Sebas 专注于一件事：在 Apple Silicon 上运行难以放进标准常驻式 runtime 的 Qwen3.5 MoE 模型。Rust CLI 只是用于 setup、demo、doctor 和 benchmark 的轻量 operational wrapper。
...
这个 custom engine 是 Sebas 存在的原因。CLI 现在只保留为 engine 和 benchmark workflow 服务的 operational wrapper。
...
| Path | Purpose |
|---|---|
| `sebas` | engine、demo、doctor、benchmark commands 的 main CLI entrypoint |
| `rust/` | 最小化的 Sebas runner CLI |
...
- engine 目前针对 Qwen3.5 MoE family assumptions。
...
Sebas 是一个独立 research project，并不隶属于、获得背书于或由 Apple、Alibaba/Qwen、ANEMLL、Hugging Face、MLX Community 或 Flash-MoE upstream projects 维护。第三方名称仅用于说明 compatibility、attribution、setup 和 benchmark context。
