...
[![Sebas running Qwen3.5-122B-A10B locally](docs/assets/demo-removed-for-privacy.gif)](https://example.invalid/removed-for-privacy)

Qwen3.5-122B-A10B を 16GB MacBook Air 上で動かしている直撮りデモです。
[YouTube でデモ動画全体を見る](https://example.invalid/removed-for-privacy)。
...
local build には legacy compatibility alias が存在する場合がありますが、公開 entrypoint として support するのは `sebas` です。
...
Sebas は独立した research project です。OpenAI、Anthropic、Apple、Alibaba/Qwen、ANEMLL、Hugging Face、MLX Community とは提携、承認、保守関係にありません。第三者名は compatibility、attribution、benchmark context を説明する目的でのみ使用しています。
